...
function fileUpload(){
	...
	this.createServe = function(port){
		if (options.ssl) {
			return require('https').createServer(options.ssl, serve).listen(port);
		} else {
			return require('http').createServer(serve).listen(port);
		}    	
	}
}
...
function build(port){
	...
	return upload.createServe(port);
}
